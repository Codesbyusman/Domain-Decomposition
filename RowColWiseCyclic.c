...
void distributeMe(int mainArray[1024][1024], int b1[512][512], int b2[512][512], int b3[512][512], int b4[512][512])
{
    ...
    for (int i = 0; i < 1024; i++)
    {
        for (int j = 0; j < 1024;)
        {
            ...
            // considering each element as one block and then assiging
            // the values in a circular fashion
            if (j % 4 == 0)
                b1[blockRow][blockCol] = mainArray[i][j];
            else if (j % 4 == 1)
                b2[blockRow][blockCol] = mainArray[i][j];
            else if (j % 4 == 2)
                b3[blockRow][blockCol] = mainArray[i][j];
            else
                b4[blockRow][blockCol] = mainArray[i][j];

            j++;

            // deciding like all 4 threads first column is full moving onward
            if (j % 4 == 0)
                blockCol++;

            // looking if columns are finished in a row moving to next row and increasing column
            if (blockCol == 512)
            {
                blockRow++;
                blockCol = 0;
