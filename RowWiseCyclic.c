...
void distributeMe(int mainArray[1024][1024], int b1[256][1024], int b2[256][1024], int b3[256][1024], int b4[256][1024])
{
    // the one that will track of rows in buffer
    int bufferRow = 0;
    ...
    for (int i = 0; i < 1024;)
    {
        for (int j = 0; j < 1024; j++)
        {
            ...
            if (i % 4 == 0)
                b1[bufferRow][j] = mainArray[i][j];
            else if (i % 4 == 1)
                b2[bufferRow][j] = mainArray[i][j];
            else if (i % 4 == 2)
                b3[bufferRow][j] = mainArray[i][j];
            else
                b4[bufferRow][j] = mainArray[i][j];
        }

        i++;

        // looking for j if added in all buffers just move to next row of eah buffer
        if (i % 4 == 0)
            bufferRow++;
    }
}
